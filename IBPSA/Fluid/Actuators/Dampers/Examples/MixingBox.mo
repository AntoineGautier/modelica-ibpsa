...
model MixingBox
 ...
 package Medium = IBPSA.Media.Air
  ...
  IBPSA.Fluid.Actuators.Dampers.MixingBox mixBox(
    ...
    IBPSA.Fluid.Sources.Boundary_pT bouSup(             redeclare package
      Medium = Medium, T=273.15 + 26,
    use_p_in=true,
    nPorts=1)                                                                       annotation (Placement(
        transformation(extent={{68,-10},{48,10}})));
    IBPSA.Fluid.Sources.Boundary_pT bouRet(             redeclare package
...
equation
  ...
  connect(bouRet.ports[1], mixBox.port_Ret) annotation (Line(
      ...
      color={0,127,255}));
  annotation (             __Dymola_Commands(file="modelica://IBPSA/Resources/Scripts/Dymola/Fluid/Actuators/Dampers/Examples/MixingBox.mos"
        "Simulate and plot"),
